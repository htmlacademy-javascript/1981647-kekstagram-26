const SpecialEffects = {
  MARVIN: 'marvin',
  PHOBOS: 'phobos',
  NONE: 'none',
};
const SliderSettings = {
  NONE: {
    class: 'effects__preview--none',
    min: 0,
    max: 0.1,
    start: 0,
    step: 0,
    effect: 'none',
  },
  CHROME: {
    class: 'effects__preview--chrome',
    min: 0,
    max: 1,
    start: 1,
    step: 0.1,
    effect: 'grayscale',
  },
  SEPIA: {
    class: 'effects__preview--sepia',
    min: 0,
    max: 1,
    start: 1,
    step: 0.1,
    effect: 'sepia',
  },
  MARVIN: {
    class: 'effects__preview--marvin',
    min: 0,
    max: 100,
    start: 100,
    step: 1,
    effect: 'invert',
  },
  PHOBOS: {
    class: 'effects__preview--phobos',
    min: 0,
    max: 3,
    start: 3,
    step: 0.1,
    effect: 'blur',
  },
  HEAT: {
    class: 'effects__preview--heat',
    min: 1,
    max: 3,
    start: 3,
    step: 0.1,
    effect: 'brightness',
  },
};

const uploadImageForm = document.querySelector('.img-upload__form');
const sliderContainerElement = uploadImageForm.querySelector('.effect-level__slider');
const sliderValue = uploadImageForm.querySelector('.effect-level__value');
const sliderEffectElement = uploadImageForm.querySelector('.img-upload__preview');
const effectsList = uploadImageForm.querySelector('.effects__list');

noUiSlider.create(sliderContainerElement, {
  range: {
    min: 0,
    max: 1,
  },
  start: 0.5,
  step: 0.1,
  connect: 'lower',
});

const setSliderDefaultSettings = () => {
  const slider = document.querySelector('.effect-level');
  slider.style.display = 'none';

  sliderEffectElement.classList.add(SliderSettings.NONE.class);
  sliderEffectElement.style.filter = SpecialEffects.NONE;
};

setSliderDefaultSettings();

const onEffectsListChange = (evt) => {

  sliderEffectElement.classList.remove(sliderEffectElement.classList[1]);

  const slider = document.querySelector('.effect-level');

  const sliderSetting = SliderSettings[evt.target.value.toUpperCase()];

  sliderEffectElement.classList.add(sliderSetting.class);

  slider.style.display = (evt.target.value === SpecialEffects.NONE) ? 'none' : 'block';

  sliderContainerElement.noUiSlider.updateOptions({
    range: {
      min: sliderSetting.min,
      max: sliderSetting.max,
    },
    start: sliderSetting.start,
    step: sliderSetting.step,
    format: {
      to: function (value) {
        if (Number.isInteger(value)) {
          return value.toFixed(0);
        }
        return value.toFixed(1);
      },
      from: function (value) {
        return parseFloat(value);
      },
    },
  });

  sliderContainerElement.noUiSlider.on('update', (values, handle) => {
    sliderValue.value = values[handle];
    if (evt.target.value === SpecialEffects.NONE) {
      sliderEffectElement.style.filter = SpecialEffects.NONE;

    } else {
      let units = '';
      switch (evt.target.value) {
        case SpecialEffects.MARVIN:
          units = '%';
          break;
        case SpecialEffects.PHOBOS:
          units = 'px';
          break;
      }
      sliderEffectElement.style.filter = `${sliderSetting.effect}(${sliderValue.value}${units})`;
    }
  });
};

effectsList.addEventListener('change', onEffectsListChange);

export { setSliderDefaultSettings };
